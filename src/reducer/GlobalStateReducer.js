const globalStateReducer = (state, action) => {
  switch (action.type) {
    ...
    case "ADD_OR_UPDATE_BET":
      ...
      const existingBetIndex = updatedTicket.bets.findIndex(
      ...
      updatedTicket.totalOdd = updatedTicket.bets.reduce(
        (acc, bet) => acc * bet.odd,
        1
      );
    ...
    case "REMOVE_BET":
      ...
      const updatedBets = state.ticket.bets.filter(
        (bet) => bet.fixtureId !== fixtureIdToRemove
      );

      // Recalculate totalOdd and totalWin based on the updated bets
      const updatedTotalOdd = updatedBets.reduce(
        (acc, bet) => acc * bet.odd,
        1
      );
    ...
    case "ADD_FIXTURES":
      ...
      console.log("=====================================================");
      console.log(
        `ACTION: Adding new Fixtures for League ID = ${fixturesToAdd.id}`
      );
    ...
    case "REMOVE_FIXTURES":
      ...
      const updatedFixtures = state.fixtures.filter(
        (fixture) => fixture.id !== leagueId
      );
    ...
    case "ADD_SELECTED_LEAGUE":
      ...
      console.log("=====================================================");
      console.log(
        `ACTION: Adding League ID = ${selectedLeague} to Selected Leagues`
      );
    ...
    case "REMOVE_SELECTED_LEAGUE":
      ...
      console.log("=====================================================");
      console.log(
        `ACTION: Removing League ID = ${leagueToRemove} from Selected Leagues`
      );

      const curtrentSelectedleagues = state.selectedLeagues.filter(
        (id) => id !== leagueToRemove
      );
    ...
    case "REMOVE_ERROR":
      ...
      const errorsAfterRemoval = clonedErrors.filter(
        (message) => !errorMessages.includes(message)
      );
